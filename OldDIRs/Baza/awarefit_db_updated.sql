...
\restrict ouUKVyyh6vjfWSrbBh8yVTASfO3VUooIEdDjvWd8HiEeZuZCilEoovz8DxPa5gk

-- Dumped from database version 18.1
-- Dumped by pg_dump version 18.1

-- Started on 2025-12-29 12:39:12
...
--
-- TOC entry 280 (class 1255 OID 24780)
...
--
-- TOC entry 283 (class 1255 OID 24787)
...
--
-- TOC entry 285 (class 1255 OID 24788)
...
BEGIN
    ...
    -- Jeśli brakuje kluczowych danych, nie liczmy błędnie
    IF v_height IS NULL OR v_waist IS NULL OR v_neck IS NULL THEN
        RETURN NULL;
    END IF;

    IF v_gender = 'Male' THEN
        v_bf := 495 / (1.0324 - 0.19077 * log(10, v_waist - v_neck) + 0.15456 * log(10, v_height)) - 450;
    ELSE
        IF v_hips IS NULL THEN RETURN NULL; END IF;
        v_bf := 495 / (1.29579 - 0.35004 * log(10, v_waist + v_hips - v_neck) + 0.22100 * log(10, v_height)) - 450;
    END IF;
...
--
-- TOC entry 284 (class 1255 OID 24784)
-- Name: calculate_user_tdee(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.calculate_user_tdee(p_user_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_gender TEXT;
    v_goal TEXT;
    v_tdee INT;
    v_base_kcal INT := 2000; -- Domyślna baza
BEGIN
    -- Pobieramy tylko te kolumny, które na pewno masz w tabeli users
    SELECT gender, goal INTO v_gender, v_goal 
    FROM public.users 
    WHERE id = p_user_id;

    -- Prosta logika zapasowa (BMR)
    IF v_gender = 'Male' THEN
        v_tdee := 2500;
    ...
    -- Korekta pod cel (Goal)
    IF v_goal = 'Cut' THEN
        v_tdee := v_tdee - 500;
    ELSIF v_goal = 'Bulk' THEN
        v_tdee := v_tdee + 300;
    END IF;
...
ALTER FUNCTION public.calculate_user_tdee(p_user_id integer) OWNER TO postgres;
...
--
-- TOC entry 281 (class 1255 OID 24782)
...
DECLARE
    ...
    v_days_active INT;
    v_has_legs BOOLEAN;
    v_has_push BOOLEAN; -- Klatka, Barki, Triceps
    v_has_pull BOOLEAN; -- Plecy, Biceps
    v_result TEXT;
BEGIN
    -- 1. Pobieramy statystyki ze WSZYSTKICH dostępnych treningów użytkownika
    SELECT 
        COUNT(DISTINCT mg.id),                       -- Ile różnych grup mięśniowych w historii
        COUNT(DISTINCT w.date::date),                -- Łączna liczba dni treningowych
        EXISTS (SELECT 1 FROM public.workouts w2 
                JOIN public.workout_exercises we2 ON w2.id = we2.workout_id
                JOIN public.exercises e2 ON we2.exercise_id = e2.id
                JOIN public.muscle_groups mg2 ON e2.muscle_group_id = mg2.id
                WHERE w2.user_id = p_user_id 
                AND mg2.name ILIKE '%Nogi%'),       -- Czy kiedykolwiek były nogi
        EXISTS (SELECT 1 FROM public.workouts w2 
                JOIN public.workout_exercises we2 ON w2.id = we2.workout_id
                JOIN public.exercises e2 ON we2.exercise_id = e2.id
                JOIN public.muscle_groups mg2 ON e2.muscle_group_id = mg2.id
                WHERE w2.user_id = p_user_id 
                AND mg2.name IN ('Klatka piersiowa', 'Barki', 'Triceps')), -- Czy kiedykolwiek był PUSH
        EXISTS (SELECT 1 FROM public.workouts w2 
                JOIN public.workout_exercises we2 ON w2.id = we2.workout_id
                JOIN public.exercises e2 ON we2.exercise_id = e2.id
                JOIN public.muscle_groups mg2 ON e2.muscle_group_id = mg2.id
                WHERE w2.user_id = p_user_id 
                AND mg2.name IN ('Plecy', 'Biceps'))   -- Czy kiedykolwiek był PULL
    INTO v_categories_count, v_days_active, v_has_legs, v_has_push, v_has_pull
    ...
    -- 2. Logika rozpoznawania na podstawie ogólnej aktywności
    IF v_days_active IS NULL OR v_days_active = 0 THEN
        v_result := 'Brak treningów';
    ELSIF v_has_push AND v_has_pull AND v_has_legs AND v_days_active >= 3 THEN
        v_result := 'Push Pull Legs';
    ELSIF v_has_push AND v_has_pull AND NOT v_has_legs THEN
        v_result := 'Push Pull';
    ELSIF v_days_active <= 3 AND v_categories_count >= 5 THEN
        v_result := 'Full Body Workout';
    ELSIF v_days_active >= 4 AND v_categories_count >= 6 THEN
        v_result := 'Split';
    ELSIF v_days_active >= 4 AND v_categories_count BETWEEN 3 AND 5 THEN
        v_result := 'Upper Lower';
    ELSE
        v_result := 'Własny plan';
    END IF;
...
--
-- TOC entry 282 (class 1255 OID 24783)
...
--
-- TOC entry 4932 (class 2604 OID 16434)
...
--
-- TOC entry 4931 (class 2604 OID 16417)
...
--
-- TOC entry 4930 (class 2604 OID 16408)
...
--
-- TOC entry 4934 (class 2604 OID 16464)
...
--
-- TOC entry 4935 (class 2604 OID 16484)
...
--
-- TOC entry 4933 (class 2604 OID 16449)
...
--
-- TOC entry 5111 (class 0 OID 16431)
-- Dependencies: 227
-- Data for Name: body_measurements; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.body_measurements (id, user_id, date, height, weight, chest, waist, biceps, thighs, hips, neck) FROM stdin;
1	1	2025-11-26 16:52:38.017785	180	85.5	105	90	38	60	100	\N
2	2	2025-11-26 16:52:38.017785	165	58	88	65	28	52	92	\N
3	3	2025-11-26 16:52:38.017785	178	92	110	95	41	65	102	\N
4	4	2025-11-26 16:52:38.017785	170	62.5	90	68	29	55	95	\N
5	5	2025-11-26 16:52:38.017785	185	78	100	82	36	58	98	\N
6	1	2025-12-29 00:00:00	180	85	\N	90	\N	\N	95	40
7	15	2025-12-29 00:00:00	175	83	107	87	40	58	99	38
\.


--
-- TOC entry 5109 (class 0 OID 16414)
-- Dependencies: 225
...
--
-- TOC entry 5107 (class 0 OID 16405)
-- Dependencies: 223
...
--
-- TOC entry 5105 (class 0 OID 16390)
-- Dependencies: 221
-- Data for Name: users; Type: TABLE DATA; Schema: public; Owner: postgres
--

COPY public.users (id, username, password, email, first_name, last_name, goal, gender) FROM stdin;
2	anna_nowak	bezpieczneHaslo	anna.n@example.com	Anna	Nowak	Maintenance	\N
3	piotr_wisniewski	silownia2023	piotr.w@example.com	Piotr	Wiśniewski	Maintenance	\N
4	kasia_lewandowska	fitness4life	kasia.l@example.com	Katarzyna	Lewandowska	Maintenance	\N
5	michal_zielinski	trening1	michal.z@example.com	Michał	Zieliński	Maintenance	\N
6	nowy_user	nowe_haslo	nowy@user.com	Test	Testowy	Maintenance	\N
7	test	test	test@example.com	Test	Test	Maintenance	\N
8	trener_arek	haslo123	arek@gym.pl	Arkadiusz	Nowak	Maintenance	\N
14	mark	gryz	marcin@gmail.com	Marcin	Fryz	Maintenance	Male
1	jan_kowalski	haslo123	jan.k@example.com	Jan	Kowalski	Maintenance	\N
15	oleiy	ylo	oleiy@gmail.com	Oliwier	Hędrzak	Bulk	Male
\.


--
-- TOC entry 5115 (class 0 OID 16461)
-- Dependencies: 231
...
--
-- TOC entry 5117 (class 0 OID 16481)
-- Dependencies: 233
...
--
-- TOC entry 5113 (class 0 OID 16446)
-- Dependencies: 229
...
39	7	2025-12-29 00:00:00
40	7	2026-01-01 00:00:00
41	7	2025-12-22 00:00:00
42	7	2025-12-25 00:00:00
43	7	2025-12-15 00:00:00
44	7	2025-12-18 00:00:00
45	7	2025-12-08 00:00:00
46	7	2025-12-11 00:00:00
47	7	2025-12-01 00:00:00
48	7	2025-12-04 00:00:00
49	7	2025-11-24 00:00:00
50	7	2025-11-27 00:00:00
51	7	2025-11-17 00:00:00
52	7	2025-11-20 00:00:00
53	7	2025-11-10 00:00:00
54	7	2025-11-13 00:00:00
55	7	2025-11-03 00:00:00
56	7	2025-11-06 00:00:00
57	7	2025-10-27 00:00:00
58	7	2025-10-30 00:00:00
59	7	2025-10-20 00:00:00
60	7	2025-10-23 00:00:00
61	7	2025-10-13 00:00:00
62	7	2025-10-16 00:00:00
64	1	2025-12-23 00:00:00
65	1	2025-12-24 00:00:00
66	1	2025-12-26 00:00:00
67	1	2025-12-28 00:00:00
68	15	2025-09-23 00:00:00
69	15	2025-10-07 00:00:00
70	15	2025-10-14 00:00:00
71	15	2025-10-21 00:00:00
72	15	2025-10-28 00:00:00
73	15	2025-11-05 00:00:00
74	15	2025-11-11 00:00:00
75	15	2025-11-18 00:00:00
76	15	2025-11-25 00:00:00
\.
...
SELECT pg_catalog.setval('public.body_measurements_id_seq', 7, true);
...
SELECT pg_catalog.setval('public.workout_exercises_id_seq', 228, true);
...
SELECT pg_catalog.setval('public.workout_sets_id_seq', 691, true);
...
SELECT pg_catalog.setval('public.workouts_id_seq', 76, true);


--
-- TOC entry 4944 (class 2606 OID 16439)
...
--
-- TOC entry 4942 (class 2606 OID 16424)
...
--
-- TOC entry 4940 (class 2606 OID 16412)
...
--
-- TOC entry 4938 (class 2606 OID 16403)
...
--
-- TOC entry 4948 (class 2606 OID 16469)
...
--
-- TOC entry 4950 (class 2606 OID 16490)
...
--
-- TOC entry 4946 (class 2606 OID 16454)
...
--
-- TOC entry 4952 (class 2606 OID 16440)
...
--
-- TOC entry 4951 (class 2606 OID 16425)
...
--
-- TOC entry 4954 (class 2606 OID 16475)
...
--
-- TOC entry 4956 (class 2606 OID 16491)
...
--
-- TOC entry 4953 (class 2606 OID 16455)
...
-- Completed on 2025-12-29 12:39:12

--
-- PostgreSQL database dump complete
--

\unrestrict ouUKVyyh6vjfWSrbBh8yVTASfO3VUooIEdDjvWd8HiEeZuZCilEoovz8DxPa5gk
